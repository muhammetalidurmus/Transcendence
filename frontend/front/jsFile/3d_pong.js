...
{   
    ...
    create_light();
    create_floor();
    l_player_p = create_player(-15);
    r_player_p = create_player(15);
    check_score();
    create_ball();
...
function cam_pos(event) // düzenlenecek1
{
    if (event.key == "1")
    {
        camera.position.set(25, 14, 0);
        camera.lookAt(0, 0, 0);
    }
    else if (event.key == "2")
    {
        camera.position.set(-30, 0, 9.25);
        camera.lookAt(0, 0, 7.75);
    }
    else if (event.key == "3")
    {
...
{
    groundGeometry = new THREE.BoxGeometry(32, 17.5, 1);
    groundMaterial = new THREE.MeshStandardMaterial({ color: 0x038C73});
    ...
    create_z_edge(0, -1, 9);
    create_z_edge(0, -1, -9);
    create_x_edge(16.5, -1, 0);
    create_x_edge(-16.5, -1, 0);
}

function create_x_edge(x, y, z)
{
    r_player_size = new THREE.BoxGeometry(1, 1, 19);
...
{
    r_player_size = new THREE.BoxGeometry(32, 1, 1);
    r_player_material = new THREE.MeshStandardMaterial({color: 0xffffff});
    r_player = new THREE.Mesh(r_player_size, r_player_material);
    r_player.receiveShadow = true;
    r_player.position.x = x;
...
{
    r_player_size = new THREE.BoxGeometry(0.5, 0.5, 3);
    r_player_material = new THREE.MeshStandardMaterial({color: 0x014034});
    r_player = new THREE.Mesh(r_player_size, r_player_material);
    scene.add(r_player);
    r_player.position.x = x;
    return r_player;
...
{
    if (event.key == "w" && l_player_p.position.z >= -6.75)
        l_player_p.position.z -= 1;
    if (event.key == "s" && l_player_p.position.z <= 6.75)
        l_player_p.position.z += 1;
    if (event.key == "o" && r_player_p.position.z >= -6.75)
        r_player_p.position.z -= 1;
    if (event.key == "l" && r_player_p.position.z <= 6.75)
        r_player_p.position.z += 1;
    console.log("-----------------------");
    console.log("player " + (l_player_p.position.x) + " " + (l_player_p.position.y) + " "  + (l_player_p.position.z));
    console.log("player " + (l_player_p.position.x) + " " + (l_player_p.position.y) + " "  + (l_player_p.position.z - 1.5));
    console.log("player " + (l_player_p.position.x) + " " + (l_player_p.position.y) + " "  + (l_player_p.position.z + 1.5));
    console.log("-----------------------");
    console.log("-----------------------");
    console.log("player " + (r_player_p.position.x) + " " + (r_player_p.position.y) + " "  + (r_player_p.position.z));
    console.log("player " + (r_player_p.position.x) + " " + (r_player_p.position.y) + " "  + (r_player_p.position.z - 1.5));
    console.log("player " + (r_player_p.position.x) + " " + (r_player_p.position.y) + " "  + (r_player_p.position.z + 1.5));
...
function move_ball() {
    ...
    if (ball.position.z + 0.5 >= 8.5 || ball.position.z - 0.5 <= -8.5) {
        ball_z *= -1;
    }

    // Skor kontrolü ve topun sıfırlanması
    if (ball.position.x + 0.5 >= 16) {
        console.log('Sol Oyuncu Skoru: ', ++leftPlayerScore);
        document.getElementById('leftPlayerScore').innerText = leftPlayerScore;
        reset_ball();
    } else if (ball.position.x - 0.5 <= -16) {
        console.log('Sağ Oyuncu Skoru: ', ++rightPlayerScore);
        document.getElementById('rightPlayerScore').innerText = rightPlayerScore;
        reset_ball();
    }
    
    if(((ball.position.x - 0.5) == (l_player_p.position.x)) && ((ball.position.z) >= ((l_player_p.position.z) - 1.5) && (ball.position.z) <= ((l_player_p.position.z) + 1.5)))
    {
        ball_x = -ball_x;
        //sound.play();
    }

    if(((ball.position.x + 0.5) == (r_player_p.position.x)) && ((ball.position.z) >= ((r_player_p.position.z) - 1.5) && (ball.position.z) <= ((r_player_p.position.z) + 1.5)))  
    {
        ball_x = -ball_x;
        //sound.play();
    }
...
{
    if (rightPlayerScore == 5 || rightPlayerScore == 5) {
        winner = leftPlayerScore > rightPlayerScore ? "Sol Oyuncu" : "Sağ Oyuncu";
        console.log("Oyun Bitti! Kazanan: " + winner);
        document.querySelector("#winner").innerHTML = "OYUN BITTİ!"
        leftPlayerScore = 0;
        rightPlayerScore = 0;
        reset_ball();
    }
